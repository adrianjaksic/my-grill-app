...
namespace BarbecueChef.MeatChoosers
{
    public class LargestAreaMeatChooserStrategy : IMeatChooserStrategy
    {
        ...
        {
            var meat1 = meats.Where(m => !m.Sorted && m.Length <= maxLength && m.Width <= maxWidth).OrderByDescending(m => m.Area).FirstOrDefault();
            var meat2 = meats.Where(m => !m.Sorted && m.Width <= maxLength && m.Length <= maxWidth).OrderByDescending(m => m.Area).FirstOrDefault();
